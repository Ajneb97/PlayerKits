...
public class V1_16_R3 {
	...
	public ItemStack setUnbreakable(ItemStack item){
		net.minecraft.server.v1_16_R3.ItemStack stack = org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_16_R3.NBTTagCompound tag = stack.hasTag() ? stack.getTag() : new net.minecraft.server.v1_16_R3.NBTTagCompound(); //Create the NMS Stack's NBT (item data)		
		tag.setByte("Unbreakable", (byte)1); //Set unbreakable value to true
		stack.setTag(tag); //Apply the tag to the item
		item = org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asBukkitCopy(stack); //Get the bukkit version of the stack
		return item;
	}
	
	public boolean getUnbreakable(ItemStack item){
		net.minecraft.server.v1_16_R3.ItemStack stack = org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_16_R3.NBTTagCompound tag = new net.minecraft.server.v1_16_R3.NBTTagCompound(); //Create the NMS Stack's NBT (item data)
		tag = stack.getTag();
	...
	public void guardarSkull(ItemStack item, String path, FileConfiguration config,String nombreJugador) {
		net.minecraft.server.v1_16_R3.ItemStack cabeza = org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_16_R3.NBTTagCompound cabezaTag = cabeza.getTag();
		if(cabeza.hasTag()){
			if(cabezaTag.hasKey("SkullOwner")){
				net.minecraft.server.v1_16_R3.NBTTagCompound skullOwner = cabezaTag.getCompound("SkullOwner");
				if(skullOwner.hasKey("Properties")){
					net.minecraft.server.v1_16_R3.NBTTagCompound propiedades = skullOwner.getCompound("Properties");
					if(propiedades.hasKey("textures")){
						net.minecraft.server.v1_16_R3.NBTTagList texturas = propiedades.getList("textures", 10);
	...
	public void guardarNBT(ItemStack item, String path, FileConfiguration config) {
		net.minecraft.server.v1_16_R3.ItemStack itemModificado = org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_16_R3.NBTTagCompound itemTag =  itemModificado.getTag();
	...
	public ItemStack setNBT(ItemStack item, String path, FileConfiguration config) {
		net.minecraft.server.v1_16_R3.ItemStack nuevoItem = org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_16_R3.NBTTagCompound tag = nuevoItem.hasTag() ? nuevoItem.getTag() : new net.minecraft.server.v1_16_R3.NBTTagCompound(); 
		...
		for(int i=0;i<listaNBT.size();i++) {
			...
			if(sep[1].startsWith("{")) {
				try {
					net.minecraft.server.v1_16_R3.NBTTagCompound tagNew = net.minecraft.server.v1_16_R3.MojangsonParser.parse(nbt.replace(id+";", ""));
		...
		return org.bukkit.craftbukkit.v1_16_R3.inventory.CraftItemStack.asBukkitCopy(nuevoItem);
	}
}
