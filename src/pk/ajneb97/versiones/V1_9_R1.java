...
public class V1_9_R1 {
	...
	public void guardarSkullDisplay(ItemStack item, String path, FileConfiguration config) {
		...
		net.minecraft.server.v1_9_R1.NBTTagCompound cabezaTag = cabeza.getTag();
		if(cabeza.hasTag()){
			if(cabezaTag.hasKey("SkullOwner")){
				net.minecraft.server.v1_9_R1.NBTTagCompound skullOwner = cabezaTag.getCompound("SkullOwner");
				String skullmeta = "";
				...
				if(skullOwner.hasKey("Properties")){
					net.minecraft.server.v1_9_R1.NBTTagCompound propiedades = skullOwner.getCompound("Properties");
					if(propiedades.hasKey("textures")){
						net.minecraft.server.v1_9_R1.NBTTagList texturas = propiedades.getList("textures", 10);
	...
	public ItemStack setUnbreakable(ItemStack item){
		...
		net.minecraft.server.v1_9_R1.NBTTagCompound tag = stack.hasTag() ? stack.getTag() : new net.minecraft.server.v1_9_R1.NBTTagCompound(); //Create the NMS Stack's NBT (item data)		
		tag.set("Unbreakable", new net.minecraft.server.v1_9_R1.NBTTagByte((byte)1)); //Set unbreakable value to true
		stack.setTag(tag); //Apply the tag to the item
		item = org.bukkit.craftbukkit.v1_9_R1.inventory.CraftItemStack.asBukkitCopy(stack); //Get the bukkit version of the stack
		return item;
	}
	
	public boolean getUnbreakable(ItemStack item){
		net.minecraft.server.v1_9_R1.ItemStack stack = org.bukkit.craftbukkit.v1_9_R1.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_9_R1.NBTTagCompound tag = new net.minecraft.server.v1_9_R1.NBTTagCompound(); //Create the NMS Stack's NBT (item data)
		tag = stack.getTag();
	...
	public ItemStack setSkull(ItemStack crafteos, String path, FileConfiguration config) {
		net.minecraft.server.v1_9_R1.ItemStack cabeza = org.bukkit.craftbukkit.v1_9_R1.inventory.CraftItemStack.asNMSCopy(crafteos);
		net.minecraft.server.v1_9_R1.NBTTagCompound tag = cabeza.hasTag() ? cabeza.getTag() : new net.minecraft.server.v1_9_R1.NBTTagCompound(); 
		net.minecraft.server.v1_9_R1.NBTTagCompound skullOwnerCompound = new net.minecraft.server.v1_9_R1.NBTTagCompound();
		net.minecraft.server.v1_9_R1.NBTTagCompound propiedades = new net.minecraft.server.v1_9_R1.NBTTagCompound();
	...
	public ItemStack setAttributes(ItemStack item, String path, FileConfiguration config){
		net.minecraft.server.v1_9_R1.ItemStack nuevoItem = org.bukkit.craftbukkit.v1_9_R1.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_9_R1.NBTTagCompound tag = nuevoItem.hasTag() ? nuevoItem.getTag() : new net.minecraft.server.v1_9_R1.NBTTagCompound(); 
		net.minecraft.server.v1_9_R1.NBTTagList attributesList = new net.minecraft.server.v1_9_R1.NBTTagList();
		path = path+".attributes";
		if(config.contains(path)){
			List<String> atributos = config.getStringList(path);
			for(int i=0;i<atributos.size();i++){
				net.minecraft.server.v1_9_R1.NBTTagCompound atributo = new net.minecraft.server.v1_9_R1.NBTTagCompound();
		...
		return org.bukkit.craftbukkit.v1_9_R1.inventory.CraftItemStack.asBukkitCopy(nuevoItem);
	}

	public void guardarAttributes(ItemStack item, String path, FileConfiguration config) {
		net.minecraft.server.v1_9_R1.ItemStack itemModificado = org.bukkit.craftbukkit.v1_9_R1.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_9_R1.NBTTagCompound itemTag =  itemModificado.getTag();
		if(itemModificado.hasTag()){
			if(itemTag.hasKey("AttributeModifiers")){
				net.minecraft.server.v1_9_R1.NBTTagList atributos = itemTag.getList("AttributeModifiers", 10);
	...
	public ItemStack setNBT(ItemStack item, String path, FileConfiguration config) {
		...
		for(int i=0;i<listaNBT.size();i++) {
			...
			if(sep[1].startsWith("{")) {
				try {
					net.minecraft.server.v1_9_R1.NBTTagCompound tagNew = net.minecraft.server.v1_9_R1.MojangsonParser.parse(nbt.replace(id+";", ""));
