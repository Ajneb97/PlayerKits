...
public class V1_8_R3 {
	...
	public void guardarSkullDisplay(ItemStack item, String path, FileConfiguration config) {
		...
		net.minecraft.server.v1_8_R3.NBTTagCompound cabezaTag = cabeza.getTag();
		if(cabeza.hasTag()){
			if(cabezaTag.hasKey("SkullOwner")){
				net.minecraft.server.v1_8_R3.NBTTagCompound skullOwner = cabezaTag.getCompound("SkullOwner");
				String skullmeta = "";
				...
				if(skullOwner.hasKey("Properties")){
					net.minecraft.server.v1_8_R3.NBTTagCompound propiedades = skullOwner.getCompound("Properties");
					if(propiedades.hasKey("textures")){
						net.minecraft.server.v1_8_R3.NBTTagList texturas = propiedades.getList("textures", 10);
						skullmeta = skullmeta+";"+texturas.get(0).getString("Value");
					}
				}
	...
	public ItemStack setUnbreakable(ItemStack item){
		net.minecraft.server.v1_8_R3.ItemStack stack = org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_8_R3.NBTTagCompound tag = stack.hasTag() ? stack.getTag() : new net.minecraft.server.v1_8_R3.NBTTagCompound(); //Create the NMS Stack's NBT (item data)		
		tag.set("Unbreakable", new net.minecraft.server.v1_8_R3.NBTTagByte((byte)1)); //Set unbreakable value to true
		stack.setTag(tag); //Apply the tag to the item
		item = org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asBukkitCopy(stack); //Get the bukkit version of the stack
		return item;
	}
	
	public boolean getUnbreakable(ItemStack item){
		net.minecraft.server.v1_8_R3.ItemStack stack = org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_8_R3.NBTTagCompound tag = new net.minecraft.server.v1_8_R3.NBTTagCompound(); //Create the NMS Stack's NBT (item data)
		tag = stack.getTag();
	...
	public ItemStack setSkull(ItemStack crafteos, String path, FileConfiguration config) {
		net.minecraft.server.v1_8_R3.ItemStack cabeza = org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asNMSCopy(crafteos);
		net.minecraft.server.v1_8_R3.NBTTagCompound tag = cabeza.hasTag() ? cabeza.getTag() : new net.minecraft.server.v1_8_R3.NBTTagCompound(); 
		net.minecraft.server.v1_8_R3.NBTTagCompound skullOwnerCompound = new net.minecraft.server.v1_8_R3.NBTTagCompound();
		net.minecraft.server.v1_8_R3.NBTTagCompound propiedades = new net.minecraft.server.v1_8_R3.NBTTagCompound();
		...
		if(cabeza.hasTag()){
			if(cabezaTag.hasKey("SkullOwner")){
				...
				if(skullOwner.hasKey("Properties")){
					...
					if(propiedades.hasKey("textures")){
						net.minecraft.server.v1_8_R3.NBTTagList texturas = propiedades.getList("textures", 10);
						config.set(path+".skull-texture", texturas.get(0).getString("Value"));
					}
	...
	public ItemStack setAttributes(ItemStack item, String path, FileConfiguration config){
		net.minecraft.server.v1_8_R3.ItemStack nuevoItem = org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_8_R3.NBTTagCompound tag = nuevoItem.hasTag() ? nuevoItem.getTag() : new net.minecraft.server.v1_8_R3.NBTTagCompound(); 
		net.minecraft.server.v1_8_R3.NBTTagList attributesList = new net.minecraft.server.v1_8_R3.NBTTagList();
		path = path+".attributes";
		if(config.contains(path)){
			List<String> atributos = config.getStringList(path);
			for(int i=0;i<atributos.size();i++){
				net.minecraft.server.v1_8_R3.NBTTagCompound atributo = new net.minecraft.server.v1_8_R3.NBTTagCompound();
		...
		return org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asBukkitCopy(nuevoItem);
	}

	public void guardarAttributes(ItemStack item, String path, FileConfiguration config) {
		net.minecraft.server.v1_8_R3.ItemStack itemModificado = org.bukkit.craftbukkit.v1_8_R3.inventory.CraftItemStack.asNMSCopy(item);
		net.minecraft.server.v1_8_R3.NBTTagCompound itemTag =  itemModificado.getTag();
		if(itemModificado.hasTag()){
			if(itemTag.hasKey("AttributeModifiers")){
				net.minecraft.server.v1_8_R3.NBTTagList atributos = itemTag.getList("AttributeModifiers", 10);
				List<String> atributosGuardados = new ArrayList<String>();
				for(int i=0;i<atributos.size();i++){
					String attributeName = atributos.get(i).getString("AttributeName");
					String name = atributos.get(i).getString("Name");
					double amount = atributos.get(i).getDouble("Amount");
					int operation = atributos.get(i).getInt("Operation");
					int uuidLeast = atributos.get(i).getInt("UUIDLeast");
					int uuidMost = atributos.get(i).getInt("UUIDMost");
					String slot = atributos.get(i).getString("Slot");
	...
	public void guardarNBT(ItemStack item, String path, FileConfiguration config) {
		...
		net.minecraft.server.v1_8_R3.NBTTagCompound itemTag =  itemModificado.getTag();	
		if(itemModificado.hasTag()){
			Set<String> tags = itemTag.c();
			List<String> listaNBT = new ArrayList<String>();
			for(String t : tags) {
				if(!t.equals("ench") && !t.equals("HideFlags") && !t.equals("display") && !t.equals("BlockEntityTag")
	...
	public ItemStack setNBT(ItemStack item, String path, FileConfiguration config) {
		...
		for(int i=0;i<listaNBT.size();i++) {
			...
			if(sep[1].startsWith("{")) {
				try {
					net.minecraft.server.v1_8_R3.NBTTagCompound tagNew = net.minecraft.server.v1_8_R3.MojangsonParser.parse(sep[1]);
